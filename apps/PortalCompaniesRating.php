...
class PortalCompaniesRating extends Controller
{
    ...
    public function index($message = null)
    {
        ...
        $sql .= " WHERE interships.intership_organization = $organization AND applications.application_status = 3";
        // Order By
    ...
    public function edit($pageName, $inputTYPE = 'id', $inputID = null, $message = null, $layout = null)
    {
        ...
        if (!is_null($inputTYPE) || !is_null($inputID)) {
            ...
            $data['student_rating'] = $this->db->select_single($this->TableRating, 'score', array('application' => $inputID));
    ...
    public function valid($type = null)
    {
        ...
        if ($type == 'rate') {
            ...
            $rules = array(
                ...
                'score' => 'required|integer|max_length:10',
            );
            ...
            $where = array('application' => $application);

            // $postData flg = 0
            $postData['application'] = $application;
            $postData['student'] = $this->db->select_single($this->Table, 'student', array('id' => $application));
            $postData['score'] = $formData['score'];

            // Input Validation Success
            if ($this->valid->validation_check($valid) === false) {

                // Check if application was completed
                $rated_application = $this->db->select_single($this->TableRating, 'application', $where);

                // Update
                if ($rated_application == $application) {
                    // Check If Already rated
                    $postData = $this->modal->load->unset($postData, array('application'));
                    ...
                    if ($this->update($postData, $where)) {
                        ...
                        //Notification
                        $this->modal->notify->set('error');
                        $message = 'System could not rate the Internship';
                    ...
                    if ($this->insert($postData)) {
                        ...
                        //Notification
                        $this->modal->notify->set('error');
                        $message = 'System could apply rate the Internship';
    ...
    public function insert($insertData, $table = null)
    {
        ...
        if ($this->auth->auth_check_level('level', $this->PageLevel)) {
            ...
            $table = (!is_null($table)) ? $table : $this->TableRating;
...
/* End of file PortalCompaniesRating.php */
