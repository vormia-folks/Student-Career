...
class PortalCompaniesProfile extends Controller
{
    ...
    public function index($message = null)
    {
        ...
        $user_details = $this->db->select($this->Table, '*', array('id' => $user_id));
        ...
        // Get Organization ID from table companies where id  => user_id
        // Then get organization name and organization website from table organizations
        $organization_id = $this->db->select_single('company', 'organization', array('id' => $user_id));
        $organization_details = $this->db->select('organizations', 'name, website', array('id' => $organization_id));
        $organization_details = (!is_null($organization_details)) ? (object) $organization_details[0] : null;
    ...
    public function edit($pageName, $inputTYPE = 'id', $inputID = null, $message = null, $layout = null)
    {
        ...
        if (!is_null($inputTYPE) || !is_null($inputID)) {
            ...
            // Get Organization ID from table companies where id  => user_id
            // Then get organization name and organization website from table organizations
            $organization_id = $this->db->select_single('company', 'organization', array('id' => $inputID));
            $organization_details = $this->db->select('organizations', 'name, website', array('id' => $organization_id));
            $organization_details = (!is_null($organization_details)) ? (object) $organization_details[0] : null;
            $data['organization_details'] = $organization_details;
...
/* End of file PortalCompaniesProfile.php */
